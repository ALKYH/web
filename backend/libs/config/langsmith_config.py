...
class StudyAbroadAgentTracer:
    ...
    def __init__(self):
        self.client = None
        self.project_name = settings.ai.LANGCHAIN_PROJECT or "AI留学规划师-默认"
    ...
    def _is_enabled(self) -> bool:
        """检查LangSmith是否启用"""
        return (
            settings.ai.LANGCHAIN_TRACING_V2 and
            settings.ai.LANGCHAIN_API_KEY is not None and
            settings.ai.LANGCHAIN_API_KEY.startswith('lsv2_')
        )
    ...
    def _initialize_client(self):
        ...
        try:
            self.client = Client(
                api_url=settings.ai.LANGCHAIN_ENDPOINT,
                api_key=settings.ai.LANGCHAIN_API_KEY
            )
    ...
    def _setup_environment(self):
        ...
        os.environ["LANGCHAIN_API_KEY"] = settings.ai.LANGCHAIN_API_KEY
        os.environ["LANGCHAIN_PROJECT"] = self.project_name
        os.environ["LANGCHAIN_ENDPOINT"] = settings.ai.LANGCHAIN_ENDPOINT
