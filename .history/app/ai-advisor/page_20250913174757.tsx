...
function AIAdvisorContent() {
  ...
  const handleQuickQuestion = (question: string) => {
    // We can't directly set the input value via `setInput` from `useChat`
    // The recommended way is to handle this via a separate state or form library
    // For simplicity, we'll simulate the input change.
    const syntheticEvent = {
      target: { value: question }
    } as React.ChangeEvent<HTMLInputElement>;
    handleInputChange(syntheticEvent);
    // You might want to focus the input field as well
  };

  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      // To submit the form, we can call handleSubmit with a form event.
      // We can get the form from the input element.
      const form = e.currentTarget.form;
      if (form) {
        // Create a synthetic event if needed by handleSubmit
                  ...
                  {messages.map(message => (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {/* Timestamps can be tricky with streaming, using creation time */}
                          {message.createdAt && (
